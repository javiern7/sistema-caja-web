...
import { fetchCashBoxes } from '../../../services/cash/cash-api';
...
export function CashHistoryPage() {
  ...
  const cashBoxesQuery = useQuery({
    queryKey: ['cash-boxes', 'history', activeContext?.id, statusFilter],
    queryFn: () =>
      fetchCashBoxes({
  ...
  const selectedCash = cashBoxes.find((cash) => String(cash.id) === selectedCashId) ?? cashBoxes[0] ?? null;
    ...
    <ResourcePageShell
      ...
      summary={
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <MetricCard helper="Cajas visibles para el filtro actual." label="Registros" value={String(cashBoxes.length)} />
        ...
        <>
          <ResourceTable<CashBoxDto>
            columns={[
              ...
              { key: 'opening', header: 'Apertura', render: (cash) => formatCurrency(cash.openingAmount) },
              { key: 'expected', header: 'Esperado', render: (cash) => formatCurrency(cash.expectedAmount) },
              { key: 'openedBy', header: 'Abierta por', render: (cash) => cash.openedByUsername ?? 'No disponible' },
              { key: 'openedAt', header: 'Fecha', render: (cash) => formatDateTime(cash.openedAt) },
            ]}
          ...
          {selectedCash ? (
            <section className="grid gap-6 lg:grid-cols-[1fr_1fr]">
              <article className="rounded-3xl border border-slate-200 bg-white p-6 shadow-soft">
                <h2 className="text-lg font-semibold text-slate-950">Detalle de caja #{selectedCash.id}</h2>
